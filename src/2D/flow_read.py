...
def read_flow():
    """
    Function used to read flow properties over time from CGNS file and save
    it in Numpy/Python file.
    """
    ...
    ijk_min = [nx_min, ny_min]
    ijk_max = [nx_max, ny_max]
    nx_range = nx_max-nx_min+1
    ny_range = ny_max-ny_min+1

    # Load O-grid coordinates (X and Y coordinates)
    x = CGNS.read_2d_coord("CoordinateX", ifile, ibase, izone, ijk_min, ijk_max, nx_range, ny_range)
    y = CGNS.read_2d_coord("CoordinateY", ifile, ibase, izone, ijk_min, ijk_max, nx_range, ny_range)
    print(f"    O-grid dimensions: {nx} x {ny}")
    print(f"    Read O-grid dimensions: {np.shape(x)[0]} x {np.shape(y)[1]}")

    # Save mesh coordinates to Python-readable format (.npz files)
    save_file = flow_path_python + 'grid.npz'
    ...
    density = np.zeros((nx_range, ny_range, nqout))        # Density accumulator
    pressure = np.zeros((nx_range, ny_range, nqout))       # Pressure accumulator
    momentumx = np.zeros((nx_range, ny_range, nqout))      # X-momentum accumulator
    momentumy = np.zeros((nx_range, ny_range, nqout))      # Y-momentum accumulator
    ...
    for i, qout in enumerate(qouts):
        ...
        density[:,:,nq] = CGNS.read_2d_flow('Density', ifile, ibase, izone, ijk_min, ijk_max, nx_range, ny_range)

        # Read and accumulate pressure field
        pressure[:,:,nq] = CGNS.read_2d_flow('Pressure', ifile, ibase, izone, ijk_min, ijk_max, nx_range, ny_range)

        # Read and accumulate momentum components
        momentumx[:,:,nq] = CGNS.read_2d_flow('MomentumX', ifile, ibase, izone, ijk_min, ijk_max, nx_range, ny_range)
        momentumy[:,:,nq] = CGNS.read_2d_flow('MomentumY', ifile, ibase, izone, ijk_min, ijk_max, nx_range, ny_range)
    ...
    # Save density fields
    file_python = flow_path_python + 'density.npy'
    # np.save(file_python, density)

    # Save pressure fields
    file_python = flow_path_python + 'pressure.npy'
    # np.save(file_python, pressure)

    # Save momentum fields
    file_python = flow_path_python + 'momentum_x.npy'
    # np.save(file_python, momentumx)
    file_python = flow_path_python + 'momentum_y.npy'
